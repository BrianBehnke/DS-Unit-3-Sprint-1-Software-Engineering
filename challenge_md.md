...
- Code review is important so that you can ensure that your code is running as intented and the outputs match the expected results. This ensures that you can reviee the code with others and they can verify the outputs with different inputs. This leads to consistency between peers as well as leave a legacy of clean code that will be able to be maintained by others over time. When code is not tested and cleaned it can be difficult for others to determine what is happening and how to update the code. 

- Containers allow for developers to have a consistent environment for their code. The environment within a container is consistently build with each tim it is loaded. There is no need to be concerned that a system or other program update will cause issues with the working code. It also allows the code to be run on different machines, as it is OS agnostic, so anyone anywhere can load up the container and work with it. 
